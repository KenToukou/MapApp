...
class CutPlaneOptimizer:
    ...
    def optimize(self):
        ...
        self.model = ConstraintAdder.apply_edge_constraint(
            x=x, V=self.vertex_num, model=self.model
        )
        # 目的関数を設定
        ...
        while True:
            ...
            # サブツアー制約を追加
            self.model, self.is_closed_circuit = ConstraintAdder.apply_add_cut(
                model=self.model, x=x, V=self.vertex_num, edges=edges
            )
